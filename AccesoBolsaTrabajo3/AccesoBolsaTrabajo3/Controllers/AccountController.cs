            ...
            if (ModelState.IsValid)
            {
                var user = await UserManager.FindAsync(model.Email, model.Password); if (user != null)
                {
                    if (user.EmailConfirmed == true)
                    {
                        ...
                        await SignInAsync(user, model.RememberMe); return Redirect("http://sagainn.com.mx:8081/BTraPfCn/" + userId);
        ...
        if (user != null)
        {
            if (user.Email == Email)
            {
                ...
                await SignInAsync(user, isPersistent: false);
                   ...
                   // RDCA Modificacion redirecion a sistema perfil 
                    var userId = user.Id;
                    return Redirect("http://sagainn.com.mx:8081/BTraPfCn/" + userId);
                }
            else
            {
                return RedirectToAction("Confirm", "Account", new { Email = user.Email });
